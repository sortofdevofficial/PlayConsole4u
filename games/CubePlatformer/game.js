...
#tj{background:rgba(91,58,247,.35);border-color:rgba(139,111,255,.6)}
#tj:active{background:rgba(91,58,247,.6)}
</style>
<div id="t-dpad">
...
async function initGame(lvlNum){
  ...
  lvl.platforms.forEach(p=>{
    ...
    if(p.falling){p._timer=0;p._fall=false;p._startY=p.y;}
  });
...
function loop(){
  ...
  for(const p of lvl.platforms){
    ...
    if(fromTop){
      ...
      player.onWall=true;player.wallDir=fromLeft?-1:1;
...
/* ─── DRAW ─── */
function draw(lvl){
  // Background gradient — now uses level's lighter bgA/bgB
  ...
  for(const fan of(lvl.windFans||[])){
    ...
    ctx.fillStyle='rgba(0,100,200,.6)';
    ctx.font='bold 12px sans-serif';
    ctx.fillText(fan.dir==='up'?'↑':fan.dir==='down'?'↓':fan.dir==='left'?'←':'→',fan.x+7,fan.y+24);
    ctx.lineWidth=1;
  }
  ...
  for(const s of(lvl.spikes||[])){
    if(hazardOn(s,s.cycle,s.phase||0)){
      ...
      for(let i=0;i<n;i++){
        const x=s.x+i*12;
        ctx.beginPath();ctx.moveTo(x,s.y+s.h);ctx.lineTo(x+6,s.y);ctx.lineTo(x+12,s.y+s.h);
        ctx.closePath();ctx.fill();
      }
    }
  }
  ...
  for(const saw of(lvl.saws||[])){
    ...
    ctx.beginPath();ctx.arc(0,0,saw.r,0,Math.PI*2);ctx.fill();
    ...
    for(let i=0;i<8;i++){
      ...
      ctx.beginPath();
      ctx.moveTo(0,0);
      ctx.lineTo(Math.cos(a)*(saw.r+5),Math.sin(a)*(saw.r+5));
      ctx.stroke();
    }
    ctx.restore();
  }
...
function drawPlayer(accent){
  ...
  if(skin.rainbow){
    ...
    rg.addColorStop(0,hsl(0));rg.addColorStop(.33,hsl(.33));
    rg.addColorStop(.66,hsl(.66));rg.addColorStop(1,hsl(1));
    ctx.fillStyle=rg;
  }else{
...
function drawHUD(lvl){
  ...
  ctx.beginPath();ctx.roundRect(10,10,320,32,10);ctx.fill();
  ctx.strokeStyle='rgba(0,0,0,.06)';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.roundRect(10,10,320,32,10);ctx.stroke();
  ctx.lineWidth=1;
  // text
  ctx.fillStyle=lvl.accent;
  ctx.font='bold 11px "DM Sans",sans-serif';
  ctx.fillText(`LV ${currentLvl}/${MAX_LEVELS}  ·  ⏱ ${t}s  ·  ${skinLabel}`,18,30);
}
...
function drawWin(secs,isRecord,prev){
  ...
  // Accent glow
  const grd=ctx.createRadialGradient(VW/2,VH/2,0,VW/2,VH/2,220);
  grd.addColorStop(0,lvl.accent+'28');
  ...
  ctx.font='bold 44px "Syne","Nunito",sans-serif';
  ctx.fillText('✓ LEVEL CLEAR!',VW/2,VH/2-60);
  ctx.shadowBlur=0;

  // Time
  ctx.fillStyle=lvl.accent==='#f4f2ee'?'#1a1625':lvl.accent;
  ctx.font='bold 28px "Syne","Nunito",sans-serif';
  ctx.fillStyle='#1a1625';
  ctx.fillText('⏱ '+secs.toFixed(3)+'s',VW/2,VH/2-14);

  // Personal best
  if(isRecord){
    ctx.fillStyle=lvl.accent;
    ctx.font='bold 13px "DM Sans","Nunito",sans-serif';
    ctx.fillText(
  ...
  ctx.font='bold 14px "DM Sans","Nunito",sans-serif';
  ctx.fillText(
    currentLvl>=MAX_LEVELS?'🎉 YOU BEAT ALL 10 LEVELS!':'Level '+(currentLvl+1)+' unlocked ★',
    VW/2, VH/2+(isRecord?42:26)
  );

  // Continue hint
  ctx.fillStyle='#a09ab0';
  ctx.font='12px "DM Sans","Nunito",sans-serif';
  ctx.fillText('Tap or click to continue',VW/2,VH/2+(isRecord?66:50));
  ctx.textAlign='left';
