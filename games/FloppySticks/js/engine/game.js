...
setTimeout(() => {
  setupTouch('btn-left', (v) => keys.a = v ? 1 : 0);
  setupTouch('btn-right', (v) => keys.d = v ? 1 : 0);
...
function loop(timestamp) {
  ...
  if (gs === 'BOT_MODE' || gs === 'ONLINE_MODE' || gs === 'MATCH_OVER') {
    ...
    for (let bi = bul.length - 1; bi >= 0; bi--) {
      const b = bul[bi];
      b.x += b.vx * delta;
      if (b.x < -40 || b.x > W + 40) { bul.splice(bi, 1); continue; }
    ...
    for (let qi = pku.length - 1; qi >= 0; qi--) {
      ...
      if (!pk.gr) {
        pk.vy += GV * delta;
        pk.y  += pk.vy * 60 * delta;
        if (pk.y >= gy - 16) { pk.y = gy - 16; pk.vy = 0; pk.gr = true; }
      }
    ...
    if (gs === 'ONLINE_MODE' && conn && conn.open && ++_nt >= 2) {
      _nt = 0;
      send({
        type: 'state', x: P.x, yRel: P.y - P.gy, vx: P.vx, vy: P.vy,
        fl: P.fl, at: P.at, gr: P.gr, sy: P.sq, ia: P.atk, as: P.asw,
        if: P.flp, fa: P.fa, wp: P.wp, rd: P.rd, hp: P.hp, ff: P.ff
      });
    }
  }
