...
export class Bamborghini {
  constructor(scene, x, z) {
    ...
    // Main body (lower chassis) - one solid piece
    add(new THREE.BoxGeometry(W, BH, L), red, 0, BY+BH/2, 0);

    // Hood (front top, attached to main body)
    add(new THREE.BoxGeometry(W-.08, 0.06, L*.28), redSh, 0, BY+BH+.03, L*.18);

    // Trunk/deck (rear top, attached to main body)
    add(new THREE.BoxGeometry(W-.1, 0.06, L*.22), redDk, 0, BY+BH+.03, -L*.22);

    // Roof pillar (B-pillar center)
    add(new THREE.BoxGeometry(W-.12, 0.06, CAB_L*.28), redDk, 0, CY+CH+.03, CAB_Z);
    ...
    // Front windscreen (angled, attached to roof)
    add(new THREE.BoxGeometry(CAB_W-.04, 0.95, 0.03), glass, 0, CY+.5, CAB_Z+CAB_L/2-.22, 0.68);
    // Rear windscreen (angled, attached to trunk)
    add(new THREE.BoxGeometry(CAB_W-.04, 0.85, 0.03), glass, 0, CY+.45, CAB_Z-CAB_L/2+.2, -0.55);

    // Side windows
    add(new THREE.BoxGeometry(0.03, .7, CAB_L*.45), glassDark, CAB_W/2-.08, CY+.4, CAB_Z, 0, 0, 0.02);
    add(new THREE.BoxGeometry(0.03, .7, CAB_L*.45), glassDark, -CAB_W/2+.08, CY+.4, CAB_Z, 0, 0, -0.02);

    // Front bumper (attached to main body)
    add(new THREE.BoxGeometry(W+.06, .14, .35), dark, 0, BY+.08, L/2+.06);
    add(new THREE.BoxGeometry(W+.02, .04, .25), chrome, 0, BY+.03, L/2+.08);
    // Rear bumper (attached to main body)
    add(new THREE.BoxGeometry(W+.04, .08, .32), dark, 0, BY+.08, -L/2+.14);
    add(new THREE.BoxGeometry(W, .04, .25), chrome, 0, BY+.04, -L/2+.16);

    // Headlights
    add(new THREE.BoxGeometry(.35, .06, .04), lightM, W/2-.25, BY+BH-.02, L/2+.01);
    add(new THREE.BoxGeometry(.35, .06, .04), lightM, -W/2+.25, BY+BH-.02, L/2+.01);
    // Headlight housing
    add(new THREE.BoxGeometry(.38, .08, .05), dark, W/2-.25, BY+BH+.01, L/2+.015);
    add(new THREE.BoxGeometry(.38, .08, .05), dark, -W/2+.25, BY+BH+.01, L/2+.015);

    // Tail lights (attached to trunk)
    add(new THREE.BoxGeometry(.45, .05, .04), brakeM, W/2-.3, BY+BH-.02, -L/2-.01);
    add(new THREE.BoxGeometry(.45, .05, .04), brakeM, -W/2+.3, BY+BH-.02, -L/2-.01);
    ...
    // Door lines
    add(new THREE.BoxGeometry(.02, .45, CAB_L*.5), redDk, CAB_W/2-.06, CY+.25, CAB_Z);
    add(new THREE.BoxGeometry(.02, .45, CAB_L*.5), redDk, -CAB_W/2+.06, CY+.25, CAB_Z);

    // Hood line
    add(new THREE.BoxGeometry(W-.1, 0.02, L*.32), redDk, 0, BY+BH+.02, L*.12);

    // Rear wing/spoiler (attached to trunk, NOT floating)
    add(new THREE.BoxGeometry(W-0.2, 0.04, 0.12), redDk, 0, BY+BH+0.08, -L/2-.02);
    add(new THREE.BoxGeometry(0.04, 0.12, 0.1), redDk, W/2-0.3, BY+BH+0.04, -L/2-.02);
    add(new THREE.BoxGeometry(0.04, 0.12, 0.1), redDk, -W/2+0.3, BY+BH+0.04, -L/2-.02);
