...
export class WobblyCharacter {
  constructor(scene) {
    ...
    this.punchTime = 0;
    this.punchKnockback = null; // {target, dir} set externally
  ...
  setDrivingState(isDriving, carPos=null, carAngle=0, steerVal=0, time=0, worldMap=null, seatSide=-1) {
    ...
    this.position.x += Math.cos(carAngle)*0.85*seatSide - Math.sin(carAngle)*.38;
    this.position.z -= Math.sin(carAngle)*0.85*seatSide + Math.cos(carAngle)*.38; // fixed sign
  ...
  update(deltaTime, time, moveDirection, worldMap=null) {
    ...
    // Punch animation — override right arm
    if (this.isPunching) {
      ...
      if (pt < 0.4) {
        ...
        const t = pt/0.4;
        this.rightArmPivot.rotation.set(-.15+t*0.5, 0, -1.05+t*.3);
      } else if (pt < 0.7) {
        // SNAP forward fast
        const t = (pt-0.4)/0.3;
        this.rightArmPivot.rotation.set(-.15+0.5-t*2.2, 0, -1.05+.3-t*.8);
        // squash body on impact
        if (t > 0.5) this._sq = 0.82;
      } else if (pt < 1.1) {
        // Follow through / recoil
        const t = (pt-0.7)/0.4;
        this.rightArmPivot.rotation.set(-1.85+t*1.7, 0, -.75+t*.3);
      } else {
        this.isPunching = false;
        this.rightArmPivot.rotation.set(-.15,0,-1.05);
      }
